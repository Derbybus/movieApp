...
public class GlobalException extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleDeviceException(VideoAlreadyExistsException ex){
        String message = ex.getMessage();
        List<String> details = new ArrayList<>();
        details.add("Error: A movie with this name already exists");
        ApiError errors = new ApiError(message,details, HttpStatus.NOT_FOUND, LocalDateTime.now());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(errors);
    }

    @ExceptionHandler(VideoNotFoundException.class)
    protected ResponseEntity<Object> handleDeviceException(VideoNotFoundException ex){
        String message = ex.getMessage();
        List<String> details = new ArrayList<>();
        details.add("Error: Movie not found");
        ApiError errors = new ApiError(message,details, HttpStatus.NOT_FOUND, LocalDateTime.now());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(errors);
    }
